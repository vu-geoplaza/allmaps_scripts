...
import pprint
from config import CDM_APIURL

def getMetadata(nick, ptr):
    url = (CDM_APIURL) + '?q=dmGetItemInfo/%s/%s/json' % (nick, ptr)
    #print(url)
...
def getCpdPages(nick, ptr):
    url = (CDM_APIURL) + '?q=dmGetCompoundObjectInfo/%s/%s/json' % (nick, ptr)
    #print(url)
    response = requests.get(url)
    return response.json()
...
def getTotalRec(nick):
    url = (CDM_APIURL) + '?q=dmQueryTotalRecs/%s|0/xml' % (nick)
    response = requests.get(url)
...
def query(nick, searchstrings, field, sortby, maxrecs, resume):
    url = (CDM_APIURL) + '?q=dmQuery/%s/%s/%s/%s/%s/%s/json' % (nick, searchstrings, field, sortby, maxrecs, resume)
    #print(url)
    response = requests.get(url)
    return response.json()
